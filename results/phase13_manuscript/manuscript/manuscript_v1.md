...
**Recommended title (from ranked candidates).** Alternate candidates are listed in the PHASE 13 final report.
...
Earlier corrected five-seed summaries are consistent with this picture after removal of obsolete mixed-coordinate artifacts, but we avoid describing Direction A as a robust “sign flip” against totalVI. Source-size sensitivity analyses indicate that matching source cell numbers partially narrows the Direction A/B gap yet does not fully explain it. Transfer conclusions are therefore context-dependent rather than universally multimodal.
...
## Data availability
PBMC public CITE-seq cohorts as used in project manifests. Lawlor data via Human Cell Atlas / ENA accessions recorded in PHASE 12B audit (`PRJEB40376` / HCA UUID in audit logs). Processed intermediates and tables are in the project `results/` tree.
