<!--
Alternative title: Disentangling protein-access and totalVI-associated gains in CITE-seq representation learning under distribution shift
-->
...
**Background:** CITE-seq representation learning can improve cell-type prediction, but gains may reflect access to paired surface proteins rather than a more complex multimodal representation. We quantified protein-access gain and the residual totalVI-associated contrast using simple RNA–protein controls and stress-tested that residual under distribution shift. **Results:** In the strict train-only development analysis (7,573/1,921 high-confidence cells), RNA PCA, concat PCA, and totalVI reached macro-F1 0.667, 0.745, and 0.741, respectively. Protein-access gain was +0.078 [cell-level conditional paired-bootstrap 95% interval 0.042, 0.131], whereas the residual totalVI-associated contrast was −0.004 [−0.060, 0.030]. Under transfer, inductive concat remained strong (0.735/0.734); feature-access-matched transductive concat reached 0.636 and 0.767, with totalVI about +0.016 and −0.113 relative to that feature-access control. Protein degradation narrowed residual totalVI-associated contrasts in settings where they were present, and training stochasticity limited precision for small contrasts. Across 10 Lawlor donors, totalVI exceeded capacity-matched scVI (scVI_matched) (mean paired Δ=+0.090; 10/10; donor means 0.794 vs 0.884), while donor-mean protein PCA and concat were stronger under protein-gated labels (0.923 and 0.942). **Conclusions:** Protein-access gain was substantial and resampling-stable, whereas the residual totalVI-associated contrast was near null and context-dependent.
...
This study makes three contributions. First, we quantify the empirical performance improvement associated with access to paired protein measurements using strong simple multimodal controls and distinguish it from the residual totalVI-associated contrast relative to that simple multimodal baseline. Second, we test whether this residual totalVI-associated contrast persists under source–target shift, feature-access-matched transductive comparison, protein degradation, and training stochasticity. Third, we evaluate reliability using uncertainty-aware selective prediction and donor-level external validation.

## Results

### Simple multimodal protein-access gain explained much of the internal improvement

The internal development analysis used 9,494 high-confidence healthy-control cells with matched RNA and protein measurements under a **strict train-only** protocol (7,573 training / 1,921 held-out). **Figure 1.** Study logic for separating protein-access gain from residual totalVI-associated contrast under a strict train-only protocol (7,573/1,921). RNA PCA (0.667) defines the RNA-only reference, concatenated RNA-protein PCA (0.745) defines the simple multimodal feature-access reference, and totalVI (0.741) defines the complex multimodal representation under study. Central quantities are \(G_{\mathrm{access}}=+0.078\) and \(G_{\mathrm{assoc}}=−0.004\) (descriptive contrasts, not causal architecture effects). \(R_{\mathrm{access}}\) is not highlighted as a primary recovery fraction.

**Figure 2.** Internal development-cohort contrasts under strict train-only information access with bootstrap intervals. RNA PCA reached 0.667, concat reached 0.745, scVI_matched reached 0.709, and totalVI reached 0.741. Cell-level conditional paired-bootstrap 95% intervals with n=10000 were \(G_{\mathrm{access}}=+0.078\) [0.042, 0.131] and \(G_{\mathrm{assoc}}=−0.004\) [−0.060, 0.030] (prop>0=0.355). The \(G_{\mathrm{assoc}}\) interval crossed zero, consistent with a near-null residual contrast. These intervals are conditional cell-resampling intervals, not biological confidence intervals.

**Figure 3.** compares source-only inductive concat, the feature-access-matched transductive concat control, scVI_matched, and totalVI across Directions A and B.
...
scVI_matched reached 0.656 in Direction A and 0.601 in Direction B. totalVI reached 0.652 in Direction A and 0.654 in Direction B. Thus, totalVI was near scVI_matched in Direction A, while Direction B showed a totalVI−scVI_matched difference of +0.052 with 19/20 seeds favoring totalVI.
...
Post-adaptation target-only degradation showed the same direction in the transfer setting where totalVI most clearly exceeded scVI_matched. Direction B totalVI declined from 0.654 to 0.621 at the highest corruption level, while clean scVI_matched was 0.601. Thus, the totalVI advantage over the clean RNA-only reference narrowed but did not disappear in that setting.
...
Latent posterior variance measured representation-level dispersion, not formal epistemic uncertainty. Direction A latent error AUROC was 0.544±0.054 for scVI_matched and 0.669±0.021 for totalVI. Direction B latent error AUROC was 0.469±0.075 for scVI_matched and 0.610±0.038 for totalVI. Under Direction B corruption, latent AUROC increased from 0.610 to 0.627 while predictive AUROC and risk-coverage summaries worsened. This divergence shows that the two uncertainty signals capture different behavior.
...
For scVI_matched macro-F1, the variance fractions were source 2.31%, seed 0.00%, source-by-seed interaction 35.45%, and residual run-level variation 62.23%. For totalVI, the corresponding fractions were source 0.00%, seed 5.62%, interaction 3.24%, and residual 91.13%. For the paired totalVI minus scVI_matched contrast, the fractions were source 5.09%, seed 12.04%, interaction 14.11%, and residual 68.75%.

These summaries explain why small residual totalVI-associated contrasts should be interpreted with repeated-run context. Direction A transfer was effectively near null, and the internal strict train-only \(G_{\mathrm{assoc}}\) bootstrap interval crossed zero with a near-null/negative point estimate. Direction B totalVI versus scVI_matched was more consistent because the mean difference was larger and 19/20 seeds favored totalVI, but the feature-access control still showed that residual totalVI advantage was not stable across transfer directions.
...
On the same donor aggregation unit (n=10), present-class macro-F1 donor means were 0.690 for RNA PCA, 0.923 for protein PCA, and 0.942 for concatenated RNA–protein PCA. Protein PCA and concat therefore exceeded totalVI under protein-gated labels when compared at the donor level. Because the Lawlor labels were protein-gated, this result is consistent with strong label–modality alignment rather than universal totalVI superiority. The donor analysis therefore supports multimodal measurement value across donors under protein-gated labels, while reinforcing the need to compare complex models with simple multimodal baselines at a matched aggregation unit.

## Discussion

Access to paired protein measurements accounted for a substantial part of the observed multimodal performance improvement. Under strict train-only evaluation, the residual totalVI-associated contrast relative to a simple RNA–protein representation was near null and imprecisely estimated, while performance contrasts in other settings remained strongly dependent on evaluation context.

The strict train-only analysis sharpened the distinction between modality access and representation strategy. The protein-access contrast remained essentially unchanged (\(G_{\mathrm{access}}=+0.078\) with a bootstrap interval entirely above zero), whereas the residual totalVI-associated contrast moved from a small positive estimate under an earlier feature-transductive deep-model analysis (\(G_{\mathrm{assoc}}\approx+0.034\)) to a near-null estimate under strict train-only fitting (\(G_{\mathrm{assoc}}=−0.004\) [−0.060, 0.030]). This pattern indicates that held-out feature access can materially alter apparent representation-level advantages and reinforces the need for explicitly matched information-access controls. It does **not** imply that totalVI is generally inferior to concat, nor that complex multimodal models are unnecessary; rather, no stable residual totalVI-associated advantage over concat was observed in the primary internal analysis. The earlier transductive comparison is retained only as a Supplementary information-access sensitivity.

Transfer analysis showed why this caution is necessary. Source-only inductive concat remained strong in both directions. totalVI exceeded scVI_matched consistently in Direction B but not Direction A. More importantly, totalVI did not retain a stable advantage over the feature-access-matched transductive concat control. Because this control matches unlabeled target features but not the learning objective or adaptation algorithm of scArches, it does not prove algorithmic equivalence; it does show that feature access alone can change the apparent residual contrast.
...
Training stochasticity limited the precision of small residual totalVI-associated contrasts. Residual run-level variation was large in the balanced replication design, especially for totalVI and paired totalVI−scVI_matched contrasts. This does not invalidate larger and direction-consistent effects, but it argues against overinterpreting small differences without matched baselines, repeated runs, and interval estimates.

Lawlor donor validation strengthened the conclusion that paired proteins can be highly valuable across biological donors. The totalVI−scVI_matched donor contrast favored totalVI in all 10 donors, but donor-mean protein PCA and concat exceeded totalVI under protein-gated labels. This is not a contradiction; it shows that when labels depend heavily on surface markers, direct protein-aligned representations can be extremely strong.

Several limitations remain. The internal and transfer analyses are PBMC-focused and classification-centered; conclusions should not be generalized to trajectory inference, perturbation response, or rare-state discovery without testing. Antibody panels are limited (14 shared internal proteins; 12 matched Lawlor proteins). Only one primary complex multimodal model (totalVI) was stress-tested against concat under matched train-only information access, with MOFA+ retained only as a historical secondary baseline; additional multimodal architectures were not newly trained in this revision. Lawlor labels are protein-gated, and only 10 biological donors are available. The feature-access-matched concat control does not reproduce scArches’ learning objective. Entry-wise zero-masking approximates protein degradation; an exploratory PCA-only marker-channel dropout sensitivity is reported in Supplementary Results and does not test totalVI under panel failure. Bootstrap intervals are conditional cell-level intervals, not biological confidence intervals. Uncertainty scores do not guarantee biological label correctness, and selective prediction changes retained cell-type composition. An earlier internal deep-model analysis used unlabeled held-out features during unsupervised representation learning (feature-transductive fitting); after this was identified, the primary analysis was replaced by a strict train-only evaluation, and the earlier result is retained only as an information-access sensitivity. This comparison illustrates that representation-learning access to evaluation features can materially affect apparent model contrasts; it is a methodological distinction between transductive unsupervised representation learning and strict train-only representation learning, not evidence of label leakage. These limits make the conclusion narrower but more reliable: protein access is a strong contributor, while residual totalVI-associated contrast must be demonstrated in each evaluation context under explicitly stated information-access conditions.

## Conclusion

Paired protein access produced a substantial and resampling-stable improvement in the evaluated CITE-seq setting. Under strict train-only fitting, the residual totalVI-associated contrast relative to a simple RNA-protein representation was near null and imprecisely estimated. Together with transfer and feature-access-matched analyses, these findings show that apparent multimodal representation gains should be interpreted relative to simple multimodal baselines and explicit information-access conditions.
...
**Lawlor external cohort.** External validation used the Lawlor blood CITE-seq dataset [13] (HCA `efea6426-510a-4b60-9a19-277e52bfa815`; ENA `PRJEB40376` / `ERP124005`). Verified object dimensions after formal preparation: **16,175** singlets; **5,207** author-labeled cells; **12,776** harmonized genes shared with the internal universe; **12** matched proteins; **10** biological donors. Gene identifiers were mapped from Ensembl IDs to gene symbols intersecting the current universe; **55** duplicated symbols were aggregated by summing counts across Ensembl rows. Protein features were alias-harmonized to the internal panel names (e.g., PD-1 ↔ CD279). The labeled evaluation subset retained the seven primary author classes used throughout (B, CD14_Mono, CD4T_Mem, CD4T_Naive, CD8T_Mem, CD8T_Naive, NK). The biological unit was the **donor** (*n* = 10). Author labels in the labeled subset are protein-gated. Batch covariate for deep models was sequencing lane (`run_identifier` / `batch`); donor was **not** supplied as a batch key. Primary donor metric: present-class macro-F1 (classes absent in a donor are excluded for that donor).
...
Unless otherwise noted, reported internal and transfer RNA-only deep results use capacity-matched scVI (**`scVI_matched`**), not the project’s PHASE-3 scVI default (`n_hidden=128`, `n_layers=1`, `gene_likelihood=zinb`, `dropout_rate=0.1`).
...
A stratified train/test split (`test_fraction=0.20`, stratify by `batch`, seed **0**) was assigned first on the combined **10,849**-cell object, yielding exact partition sizes **8,679** train / **2,170** test. High-confidence restriction (`annotation_tier_l2 == "high"`) was applied second within that split, giving the development evaluation cohorts **7,573** train / **1,921** test (9,494 HC cells total).

**Primary internal protocol (strict train-only; matched information access).** RNA PCA, protein PCA, concat PCA, scVI_matched, and totalVI unsupervised fits used **only** the **7,573** high-confidence training cells. Held-out evaluation cells (**1,921**) were encoded **after** fitting using frozen transforms/weights (PCA transform; scVI/totalVI `get_latent_representation` with transferred AnnData setup and **no** query adaptation training). Classifier training used training labels only; held-out labels were used only for scoring. Verified hyperparameters were preserved (`n_latent=20`, matched capacity, `max_epochs=200`, early-stopping patience 45, `batch_size=256`, totalVI `lr=4e-3`, etc.). Artifacts: `results/manuscript_submission_revision/strict_train_only/`.

**Supplementary sensitivity (historical feature-transductive deep models).** An earlier pipeline trained scVI_matched/totalVI on all **10,849** cells’ features while PCA remained train-only; that unequal-access evaluation is retained only as a Supplementary information-access sensitivity (previous G_assoc≈+0.034 [−0.012, 0.073]; totalVI 0.779) and is **not** the primary internal result.

**Transfer exception.** Feature-access-matched transductive concat intentionally uses unlabeled target features for unsupervised fitting only; target labels remain withheld until scoring.
...
\(G_{\mathrm{assoc}}\) is a **descriptive residual totalVI-associated contrast**, not a causal architecture effect: totalVI and concatenated PCA differ in objective, optimization, and representation class. \(R_{\mathrm{access}}\) is a secondary descriptive ratio only. Because numerator and denominator are recomputed jointly across bootstrap resamples, \(R_{\mathrm{access}}\) is not constrained to [0,1] and is not a recovery fraction, causal mediation proportion, variance-explained quantity, or mechanistic decomposition. Under the primary strict train-only analysis, \(R_{\mathrm{access}}\) was approximately 1.05 because the concat point estimate slightly exceeded totalVI; it was therefore not emphasized in the Abstract or Figure 1.

Primary (strict train-only) point estimates: RNA PCA 0.667, concat 0.745, totalVI 0.741; \(G_{\mathrm{access}}=+0.078\), \(G_{\mathrm{assoc}}=−0.004\), \(Gap_{\mathrm{total}}=+0.074\); secondary descriptive \(R_{\mathrm{access}}\approx1.05\).
...
Balanced computational design: **5** source subsets × **5** model seeds × **2** run replicates (Direction A focus in PHASE 12A), with paired Δ = totalVI − scVI_matched. For representation *a*, outcomes were modeled as
\[
Y_{ijr}^{(a)}=\mu^{(a)}+S_i^{(a)}+G_j^{(a)}+(SG)_{ij}^{(a)}+\varepsilon_{ijr}^{(a)},
\]
and paired differences as \(\Delta_{ijr}=Y_{ijr}^{(\mathrm{totalVI})}-Y_{ijr}^{(\mathrm{scVI\_matched})}\). Here source \(S\) is computational source-subset variation, seed \(G\) is computational model-seed variation, \((SG)\) is the source×seed interaction, and residual \(arepsilon\) is run-level computational variation; **none** are biological variance components. Variance fractions were estimated by method-of-moments expected mean squares.

### Lawlor donor-level external validation

Five deterministic folds held out **two** donors each so that every donor was a target exactly once; **10** computational seeds per deep-model fold yielded 50 matched scVI_matched–totalVI fold×seed comparisons (computational consistency, e.g. latent AUROC 38/50). Donor-level present-class macro-F1 used seed-averaged within-donor scores for deep models. Simple baselines (RNA/protein/concat PCA) used source-donor-only unsupervised fits and were scored per target donor, then summarized across the same **10** biological donors. Preferred main-text / Figure 6 aggregation unit is therefore the **donor** for all five representations. Donor-mean present-class macro-F1 values were **0.690** (RNA PCA), **0.923** (protein PCA), **0.942** (concat), **0.794** (scVI_matched), and **0.884** (totalVI). Paired totalVI−scVI_matched donor Δ mean ≈ **+0.090** (median ≈ 0.089; SD ≈ 0.010; IQR ≈ 0.013; min/max ≈ +0.077/+0.110; 10/10 donors; exact two-sided sign test *P*=0.00195). Supplementary Results also retain historical fold-mean PCA aggregates (protein ≈ 0.927; concat ≈ 0.945) for continuity, but side-by-side numerical claims in the main text use donor-level means.

### Software and reproducibility

Analyses used Python **3.11.7**, scvi-tools **1.3.3**, scikit-learn **1.5.2**, scanpy **1.10.3**, anndata **0.11.4**, PyTorch **2.14.0** (verified via project environment logs / `pip show`; CUDA builds used for PHASE 10 server runs; local Mac logs report MPS rather than CUDA), numpy **1.26.4**, pandas **2.2.2**, scipy **1.13.1**, and mofapy2 **0.7.5**. Environment files: `environment.yml`, `requirements.txt`. Code: https://github.com/Jizhuy/robust-multiomics-integration. An immutable archival release and DOI will accompany the submitted version.
...
**Figure 1.** Study logic for separating protein-access gain from residual totalVI-associated contrast under a strict train-only protocol (7,573/1,921). RNA PCA defines the RNA-only reference, concatenated RNA-protein PCA defines the simple multimodal feature-access reference, and totalVI defines the complex multimodal representation under study. Central question: how much of the observed multimodal gain is associated with protein access, and how much residual improvement remains for totalVI beyond a simple RNA–protein representation? The quantities \(G_{\mathrm{access}}\), \(G_{\mathrm{assoc}}\), and \(R_{\mathrm{access}}\) are descriptive contrasts, not causal architecture effects.

**Figure 2.** Internal development-cohort contrasts under strict train-only information access with bootstrap intervals. RNA PCA reached 0.667, concat reached 0.745, and totalVI reached 0.741. Cell-level conditional paired-bootstrap 95% intervals with n=10000 were \(G_{\mathrm{access}}=+0.078\) [0.042, 0.131], \(G_{\mathrm{assoc}}=−0.004\) [−0.060, 0.030], and total gap +0.074 [0.029, 0.130]. The \(G_{\mathrm{assoc}}\) interval crossed zero. These intervals are conditional cell-resampling intervals, not biological confidence intervals.

**Figure 3.** Transfer comparison across source-only inductive concat, feature-access-matched transductive concat control, scVI_matched, and totalVI. Inductive concat reached 0.735/0.734, feature-access transductive concat reached 0.636/0.767, scVI_matched reached 0.656/0.601, and totalVI reached 0.652/0.654 for Directions A/B (Directions A/B; computational seeds for deep models as in Methods). totalVI minus transductive concat was approximately +0.016 in Direction A and −0.113 in Direction B. The feature-access control matches unlabeled target features but not the learning objective or adaptation algorithm of scArches.

**Figure 4.** Protein degradation experiments. Training-time degradation shows mean±SD across five mask/retraining replicates at nonzero corruption levels, with model_seed fixed at 0 and a single clean run at 0.00. Values were 0.779, 0.771±0.010, 0.758±0.012, 0.760±0.012, 0.750±0.016, 0.736±0.020, and 0.755±0.011 across increasing corruption. High-corruption non-monotonicity is interpreted as within mask/retraining variability. Post-adaptation Direction B totalVI declined from 0.654 to 0.621, with clean scVI_matched at 0.601.
...
**Figure 6.** Lawlor donor-level external validation (aggregation unit = biological donor, *n*=10). Panel A shows donor-mean ± SD present-class macro-F1 for RNA PCA (0.690), protein PCA (0.923), concat (0.942), scVI_matched (0.794), and totalVI (0.884). Panel B shows paired donor differences totalVI−scVI_matched (10/10 positive; exact two-sided sign test *P*=0.00195; mean Δ≈+0.090). Protein-aligned PCA baselines exceed totalVI under protein-gated labels. Error bars are across-donor SDs, not biological confidence intervals for a super-population of donors.
...
|---|---:|---:|---|
| RNA PCA | RNA | 0.667 | RNA-only reference (train-only) |
| Protein PCA | Protein | 0.512 | Protein-only secondary baseline (train-only) |
| Concatenated RNA-protein PCA | RNA + protein | 0.745 | Simple multimodal protein-access reference (train-only) |
| MOFA+ | RNA + protein | 0.549 | Historical secondary baseline (not re-fit train-only) |
| scVI_matched | RNA | 0.709 | Capacity-matched RNA-only deep (train-only) |
| totalVI | RNA + protein | 0.741 | Multimodal deep representation (train-only) |

### Table 2. Core internal contrasts with bootstrap intervals

| Contrast | Definition | Estimate | 95% interval | prop>0 | Interpretation |
|---|---|---:|---:|---:|---|
| \(G_{\mathrm{access}}\) | \(F1_{\mathrm{concat}} - F1_{\mathrm{RNA\,PCA}}\) | +0.078 | [0.042, 0.131] | 1.0 | Protein-access gain |
| \(G_{\mathrm{assoc}}\) | \(F1_{\mathrm{totalVI}} - F1_{\mathrm{concat}}\) | −0.004 | [−0.060, 0.030] | 0.355 | Residual totalVI-associated contrast; near null; interval crosses zero |
| \(Gap_{\mathrm{total}}\) | \(F1_{\mathrm{totalVI}} - F1_{\mathrm{RNA\,PCA}}\) | +0.074 | [0.029, 0.130] | 1.0 | Total RNA-to-totalVI gap |
| \(R_{\mathrm{access}}\) | \(G_{\mathrm{access}} / Gap_{\mathrm{total}}\) | ≈1.05 | [0.680, 2.304] | NA | Descriptive ratio; not constrained to [0,1] |

Footnote: Primary values use the strict train-only protocol. Intervals are cell-level conditional paired-bootstrap 95% intervals with n=10000 resamples over evaluated cells. They are not biological confidence intervals. \(R_{\mathrm{access}}\) is a secondary descriptive ratio only (not constrained to [0,1]; not a recovery fraction).

### Table 3. Transfer contrasts

| Direction | Inductive concat | Feature-access transductive concat | scVI_matched | totalVI | totalVI - scVI_matched | totalVI - transductive concat |
|---|---:|---:|---:|---:|---:|---:|
...
| Direction A latent AUROC, scVI_matched / totalVI | 0.544±0.054 / 0.669±0.021 |
| Direction B latent AUROC, scVI_matched / totalVI | 0.469±0.075 / 0.610±0.038 |
...
| scVI_matched variance fractions | source 2.31%; seed 0.00%; interaction 35.45%; residual 62.23% |
| totalVI variance fractions | source 0.00%; seed 5.62%; interaction 3.24%; residual 91.13% |
| totalVI−scVI_matched variance fractions | source 5.09%; seed 12.04%; interaction 14.11%; residual 68.75% |

### Table 6. Lawlor donor-level validation

| Metric | Value |
|---|---:|
| Biological donors (aggregation unit) | 10 |
| totalVI − scVI_matched donor-level mean Δ | ≈+0.090 |
| Donors favoring totalVI over scVI_matched | 10/10 |
| Exact two-sided donor-level sign test | *P*=0.00195 |
| RNA PCA donor-mean present-class macro-F1 | 0.690 |
| Protein PCA donor-mean present-class macro-F1 | 0.923 |
| Concat PCA donor-mean present-class macro-F1 | 0.942 |
